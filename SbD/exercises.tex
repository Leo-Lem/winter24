...
\begin{exercise}{Meet-in-the-Middle-Angriff auf 2-DES}
  Um der geringen Schlüssellänge von 56 Bit des DES-Algorithmus zu begegnen, wurde vorgeschlagen, den
  Algorithmus mehrfach hintereinander mit verschiedenen Schlüsseln anzuwenden. Eine schlechte Lösung ist
  jedoch die lediglich zweifache Hintereinanderanwendung 2-DES.
  \begin{enumerate}
    \item Vergleichen Sie die Schlüssellänge des 2-DES-Algorithmus mit der tatsächlich erreichten Sicherheit.
    \item Sie haben folgendes Klartext/Schlüsseltext-Paar \( (m, c) \) einer 2-DES-Verschlüsselung \( E_{k_2}(E_{k_1}(m)) = c \)
    erlangt: 
    \[
    ...
    Hinweis: Beachten Sie das Beispiel zum Ablesen der Tabelle.
...
\begin{exercise}{Unsicherheit des Electronic-Codebook-Modus}
  ...
  \begin{enumerate}
    ...
    \item Veranschaulichen Sie das Problem, indem Sie ein Programm entwickeln, welches eine einfache Bitmap-
    Grafik einliest, den Bildinhalt mit dem AES-Verfahren im ECB-Modus verschlüsselt und wieder als
    Bitmap ausgibt. Rufen Sie zum Vergleich die Verschlüsselung noch einmal im CBC-Modus auf und
    vergleichen Sie die erhaltenen Resultate. Informieren Sie sich dazu über das Datenformat von BMP-
    Dateien und die Verwendung einer geeigneten Kryptobibliothek (z. B. die Java-Cryptography-API
    unter \url{http://www.ibm.com/developerworks/java/tutorials/j-sec1/section4.html} oder die Python-pycrypto-
    API unter \url{https://www.dlitz.net/software/pycrypto/api/current/}).
  \end{enumerate}
...
\begin{exercise}{Elliptische Kurven: Rechenbeispiele zur Punktaddition}
  Gegeben sei die elliptische Kurve \( E(\mathbb{GF}(p)) \) mit \( a = 1, b = 6, p = 11 \). Hinweis: Wir betrachten für diese Aufgabe die in der Kryptographie häufig genutzten elliptischen Kurven über \( \mathbb{Z}_p \) der Form \( f(x, y) : y^2 \equiv x^3 + ax + b \mod p \).
  \begin{enumerate}
  ...
  \begin{solution}
    \begin{enumerate}
        \item Punkte auf der elliptischen Kurve \( y^2 = x^3 + x + 6 \mod 11 \):
        \[
        \{(2, 4), (5, 2), (7, 6), (8, 3)\}
        \]
        \item Beispiel Punktaddition:
        \[
        P = (2, 4), Q = (5, 2) \quad \text{Ergebnis: } P + Q = (7, 6)
        \]
        \item Skalare Multiplikation:
        \[
        3 \cdot (2, 4) = (8, 3)
        \]
        Vorgehen: Addieren Sie \( P \) wiederholt zu sich selbst.
    \end{enumerate}
  \end{solution}
\end{exercise}
...
\begin{exercise}{Forward Secrecy mit RSA}
  ...
  \begin{solution}
    \begin{enumerate}
        \item Forward Secrecy wird durch kurzlebige (ephemeral) RSA-Schlüssel erreicht. Jede neue Verbindung erhält ein neues Schlüsselpaar.
        \item Wenn ein Angreifer den privaten Schlüssel erhält, kann er vergangene Verbindungen entschlüsseln, falls diese nicht mit ephemeral-Schlüsseln abgesichert wurden.
        \item Ein Angreifer kann die Kommunikation entschlüsseln, indem er den abgefangenen Schlüsseltext mit dem kompromittierten privaten Schlüssel entschlüsselt.
        \item Forward Secrecy kann durch regelmäßiges Rotieren der Schlüssel gewährleistet werden. Alte Schlüssel werden sicher gelöscht.
    \end{enumerate}
  \end{solution}
\end{exercise}
...
\begin{exercise}{Diffie-Hellman-Schlüsselaustauschprotokoll}
  ...
  \begin{solution}
    \begin{enumerate}
        \item Ablauf:
        \[
        g = 5, p = 23, a = 6, b = 15 \quad \text{(geheime Schlüssel)}
        \]
        Öffentliche Schlüssel:
        \[
        A = g^a \mod p = 8, \quad B = g^b \mod p = 19
        \]
        Gemeinsamer Schlüssel:
        \[
        K = B^a \mod p = 2 = A^b \mod p
        \]
...
\begin{exercise}{Indeterministische Verschlüsselung}\label{ex:indeterministic}
  ...
  \begin{solution}
    \begin{enumerate}
        ...
        \item Bei einem asymmetrischen Verschlüsselungssystem kann indeterministische Verschlüsselung durch die Einbeziehung eines Zufallswerts (z. B. Padding mit Zufallszahlen) in die Nachricht erreicht werden.
...
\begin{exercise}{Informationstheoretische Sicherheit}\label{ex:otp}
  ...
  \begin{solution}
    \begin{enumerate}
        \item Der Angreifer erfährt bei einem One-Time-Pad nichts über den Klartext, da der Schlüsseltext rein zufällig ist und keine Informationen über den Klartext enthält.
        \item Sicherheit des Verfahrens:
        \[
        H(M|C) = H(M) \quad \text{(Entropie bleibt erhalten).}
        \]
        Der Schlüsseltext enthält keine Informationen über den Klartext ohne den Schlüssel.
        \item Wiederverwendung des Schlüssels:
        \[
        C_1 \oplus C_2 = M_1 \oplus M_2
        \]
        Dies ermöglicht die Rekonstruktion der Klartexte \( M_1, M_2 \).
        ...
        \item Ein System: Der Sender erzeugt einen zufälligen Schlüssel und XORt diesen mit der Nachricht, um den MAC zu erstellen. Der Empfänger überprüft, ob der MAC gültig ist.
        \item Es gibt keine informationstheoretisch sicheren asymmetrischen Verfahren, da der öffentliche Schlüssel immer Angriffsvektoren bietet. Die Sicherheit ist hier nur computational.
    \end{enumerate}
  \end{solution}
\end{exercise}
...
\begin{exercise}{Unsicherheit der klassischen Verschiebechiffre}\label{ex:vigenere}
  ...
  \begin{solution}
    \begin{enumerate}
        \item Verschlüsselung mit PRUEFUNG als Schlüssel:
        \[
        C = \texttt{ILNUHAZKA\_VLA\_ANL\_KLU\_LTS\_GMMI\_VQK\_TVT\_OZUB\_DRNT\_OZXEU}
        \]
        \item Kryptanalyse:
        \begin{itemize}
        ...
        \end{itemize}
        \item Verlängerung des Schlüsselworts:
        \[
        \text{Faktor der Vergrößerung: } 27^s
        \]
...
\begin{exercise}{Klassische Chiffren und Gütekriterien}
  ...
  \begin{solution}
    \begin{enumerate}
        ...
        \begin{itemize}
            \item Mit der Skytala (\(k = 4\), \(4 \times 8\) Matrix):
            \[
            \texttt{PRUEFUNGSSPASS\_OHNE\_KOMPROMISSE} \rightarrow \texttt{PHSOROPRUNFEINSUPG\_GHNOM\_}
            \]
            \item Mit der Caesar-Chiffre (\(k = 3\)):
            \[
            \texttt{STUDIUMSFREUDE} \rightarrow \texttt{VWXGLXPVIKUHGH}
            \]
        \end{itemize}
        ...
        \end{itemize}
        \item Kombination der beiden Verfahren:
        \begin{itemize}
            \item Kombination 1 (Caesar vor Skytala): Sicherheit erhöht sich, da Buchstabenhäufigkeiten durch die Caesar-Chiffre maskiert werden.
            \item Kombination 2 (Skytala vor Caesar): Sicherheit bleibt gleich wie bei der Skytala alleine, da Caesar die Buchstabenhäufigkeiten nicht beeinflusst.
        \end{itemize}
...
\begin{exercise}{Hashfunktionen und Signaturen}
  ...
  \begin{solution}
    \begin{enumerate}
        \item Die Identitätsfunktion \( H(x) = x \) ist keine kryptographische Hashfunktion, da sie nicht kollisionsresistent ist und keine Kompression durchführt.
        ...
        \item Vorteile des Signierens von Hashwerten:
        \begin{itemize}
            \item Reduziert den Rechenaufwand bei großen Nachrichten.
            \item Stellt sicher, dass die Nachricht authentisch ist.
            \item Padding (z. B. bei RSA) schützt vor Angriffen.
        \end{itemize}
...
\begin{exercise}{Das Feistel-Prinzip}\label{ex:feistel}
  ...
  \begin{solution}
    \begin{enumerate}
        \item Zwei Runden Feistel-Verschlüsselung:
        \begin{itemize}
            \item Klartext: \( 0011 \ 0100 \)
            \item Runde 1:
            \[
...
\begin{exercise}{RSA: Eigenschaften und Sicherheit}\label{ex:rsa}
  ...
  \begin{solution}
    \begin{enumerate}
        \item Der öffentliche Exponent \( c \) muss teilerfremd zu \( \phi(n) \) sein, damit ein inverser Schlüssel existiert.
        \item Eine Nachricht \( m \) darf \( p \) oder \( q \) nicht enthalten, da dies den privaten Schlüssel direkt offenbart.
        \item Berechnung des privaten Exponenten \( d \):
        \[
        c = 5, \quad n = 7 \cdot 17 = 119, \quad \phi(n) = 96
        \]
        \( d = c^{-1} \mod \phi(n) \implies d = 77 \)
        \item Multiplikative Homomorphie:
        \[
        m_1 \cdot m_2 \mod n \implies c(m_1 \cdot m_2) = c(m_1) \cdot c(m_2)
        \]
        Angreifer können Produkte von Nachrichten entschlüsseln.
        \item Eve nutzt:
        \[
        r \cdot s \cdot r^{-1} \mod n = s
        \]
        Der Angriff ist erfolgreich.
        \item Multiplikative Homomorphie wird in homomorpher Verschlüsselung absichtlich genutzt (z. B. für Berechnungen auf verschlüsselten Daten).
    \end{enumerate}
  \end{solution}
\end{exercise}
...
\begin{exercise}{Würfeln mit RSA}
  ...
  \begin{solution}
    \begin{enumerate}
        \item Eve führt einen Chosen-Plaintext-Angriff durch:
        \[
        c_B = m^e \mod n
        \]
        Für alle möglichen \( m \), vergleicht sie den berechneten \( c_B \) mit dem übertragenen Wert, bis \( m = 3 \) gefunden wird.
...
\begin{exercise}{Forward Secrecy mit RSA}
  ...
  \begin{solution}
    \begin{enumerate}
        \item Forward Secrecy mit RSA wird erreicht, indem für jede neue Sitzung ein neues, kurzlebiges (ephemeres) RSA-Schlüsselpaar generiert wird. Der öffentliche Schlüssel wird dem Kommunikationspartner zur Verfügung gestellt, und der private Schlüssel wird nach Abschluss der Sitzung sicher gelöscht. Auf diese Weise kann ein kompromittierter langfristiger Schlüssel vergangene Sitzungen nicht gefährden.
        \item Wenn ein Angreifer den privaten Entschlüsselungsexponenten eines RSA-Schlüssels erlangt, kann er alle Nachrichten entschlüsseln, die mit dem zugehörigen öffentlichen Schlüssel verschlüsselt wurden. Dadurch wird die Vertraulichkeit der Kommunikation vollständig aufgehoben.
        \item Der Angreifer kann die verschlüsselten Nachrichten abfangen und mit dem privaten Schlüssel entschlüsseln. Alle vergangenen und zukünftigen Nachrichten, die denselben Schlüssel verwenden, können entschlüsselt werden, sofern sie aufgezeichnet wurden.
        \item Durch regelmäßiges Wechseln des privaten Schlüssels und Löschen des alten privaten Schlüssels nach Ablauf einer Sitzung wird Forward Secrecy gewährleistet. Zusätzlich kann der Diffie-Hellman-Schlüsselaustausch verwendet werden, um symmetrische Sitzungsschlüssel auszuhandeln, die nicht mit den RSA-Schlüsseln gespeichert werden.
    \end{enumerate}
  \end{solution}
\end{exercise}
...
\begin{exercise}{Elliptische Kurven: EC-ElGamal}\label{ex:elliptic}
  ...
  \begin{solution}
    \begin{enumerate}
        \item Anpassung des ElGamal-Kryptosystems für elliptische Kurven:
        \begin{itemize}
            \item **Schlüsselgenerierung:** 
            \[
            \text{Privater Schlüssel } d, \text{ öffentlicher Schlüssel } P = d \cdot G, \text{ wobei } G \text{ ein Basispunkt ist.}
            \]
            \item **Verschlüsselung:** 
            \[
            \text{Nachricht } m, \text{ Zufallswert } k, \text{ Schlüsseltext: } (C_1, C_2) = (k \cdot G, m + k \cdot P).
            \]
        ...
        \item Schlüsselgenerierung:
        \begin{itemize}
            \item Wähle \( d = 3 \) als privaten Schlüssel.
            \item Berechne \( P = d \cdot G = 3 \cdot (2, 7) = (8, 3) \) (Punkt auf der Kurve).
            \item Verschlüsselung der Nachricht \( m = (5, 2) \):
            \[
            C_1 = k \cdot G = 2 \cdot (2, 7) = (6, 3), \quad C_2 = m + k \cdot P = (5, 2) + 2 \cdot (8, 3) = (7, 6).
            \]
            Schlüsseltext: \( (C_1, C_2) = ((6, 3), (7, 6)) \).
        \end{itemize}
        \item Entschlüsselung:
        \[
        m = C_2 - d \cdot C_1 = (7, 6) - 3 \cdot (6, 3) = (5, 2).
        \]
        Die ursprüngliche Nachricht \( m \) wurde korrekt wiederhergestellt.
        \item Die Sicherheit des Verfahrens beruht auf dem **diskreten Logarithmusproblem** über elliptischen Kurven. Ein Angreifer müsste \( k \) aus \( C_1 = k \cdot G \) berechnen, was aufgrund der mathematischen Schwierigkeit des Problems praktisch unmöglich ist.
    \end{enumerate}
  \end{solution}
\end{exercise}
...
\begin{exercise}{Verbindungsverschlüsselung in GSM}
  ...
  \begin{solution}
    \begin{itemize}
        \item \textbf{Verschlüsselte Abschnitte:} Der Abschnitt zwischen dem Mobiltelefon und der Basisstation wird durch den A5-Algorithmus verschlüsselt.
        \item \textbf{Unverschlüsselte Abschnitte:} Die Verbindung zwischen der Basisstation und den zentralen Netzwerkkomponenten (z. B. dem MSC) ist unverschlüsselt. Dies stellt ein Sicherheitsrisiko dar, da Angriffe auf das Netzwerk (z. B. durch gefälschte Basisstationen) möglich sind.
    \end{itemize}
  \end{solution}
\end{exercise}
...
\begin{exercise}{Betriebsarten}
  ...
  \begin{solution}
    \begin{enumerate}
        \item Die Modi ECB und CBC benötigen eine invertierbare Verschlüsselungsfunktion, da der Klartext beim Entschlüsseln zurückgewonnen werden muss. Der CTR-Modus hingegen benötigt keine invertierbare Funktion, da er auf dem XOR-Mechanismus basiert.
        \item Die Verschlüsselung lässt sich im CTR-Modus parallelisieren, da die Blöcke unabhängig voneinander verarbeitet werden können. Im CBC-Modus ist dies nicht möglich, da jeder Block von seinem Vorgänger abhängt.
        \item Ein Bitfehler im zweiten Block beeinflusst nur den entsprechenden Block im ECB-Modus. Im CBC-Modus beeinflusst er zusätzlich den nachfolgenden Block.
        \item Wenn ein Bit im zweiten Block fehlt, kann nur der ECB-Modus korrekt entschlüsseln, da die Blöcke unabhängig verarbeitet werden. Im CBC- und CTR-Modus führt der Fehler zu Synchronisationsverlust.
        \item Beim Verlust eines ganzen Blocks kann nur der ECB-Modus korrekt entschlüsseln, da die Blöcke unabhängig verarbeitet werden.
        \item Im CTR-Modus kann ein Angreifer zwei Klartexte entschlüsseln, wenn der gleiche IV verwendet wurde:
        \[
        C_1 \oplus C_2 = M_1 \oplus M_2.
        \]
        Durch eine bekannte Klartextanalyse kann \( M_2 \) bestimmt werden.
    \end{enumerate}
  \end{solution}
\end{exercise}
...
\begin{exercise}{Elliptische Kurven}
  ...
  \begin{solution}
    \begin{enumerate}
        \item Die Sicherheit elliptischer Kurven beruht auf dem Problem des diskreten Logarithmus in elliptischen Kurvengruppen, das als schwierig gilt.
        \item Punkte auf der elliptischen Kurve \( y^2 = x^3 + 6x + 1 \mod 11 \):
        \[
        \{(2, 4), (5, 2), (7, 6), (8, 3), \ldots\}.
        \]
        \item Vorteile von ECC:
        \begin{itemize}
            \item Kürzere Schlüssel bei gleicher Sicherheit (z. B. ECC-256 entspricht RSA-3072).
  ...
  \begin{solution}
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            \item Angreifer können Schlüssel bei der Erstkommunikation abfangen.
        \end{itemize}
    \end{enumerate}
  \end{solution}
\end{exercise}
...
\begin{exercise}{GSM}
  ...
  \begin{solution}
    \begin{enumerate}
        \item Abkürzungen und ihre Funktion:
        \begin{itemize}
            \item \( K_i \): Geheimer Authentifizierungsschlüssel.
            \item \( IMSI \): Internationale Teilnehmerkennung.
            \item \( RAND \): Zufallszahl für Authentifizierung.
            \item \( SRES \): Signatur zur Authentifizierung.
            \item \( LAI \): Standortbereichskennung.
            \item \( MSISDN \): Telefonnummer des Teilnehmers.
            \item \( K_c \): Verschlüsselungsschlüssel für den Funkkanal.
        \end{itemize}
        \item Viele der Funktionen wie \( K_i \), \( RAND \) und \( K_c \) finden sich auch im UMTS wieder, jedoch oft mit anderen Algorithmen oder Benennungen.
    \end{enumerate}
  \end{solution}
\end{exercise}
