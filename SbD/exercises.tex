...
\begin{exercise}{Kommutative Verschlüsselung}
  ...
  \end{align*}
  \begin{enumerate}
    \item Zeigen Sie, dass folgende Verschlüsselungen kommutativ entsprechend der obigen Definition sind:
      \begin{enumerate}
      ...
      \end{enumerate}
    \item Welche weitere Kommutativitätseigenschaft weisen CTR und RSA mit den Spezifikationen aus a) auf?
  ...
  \begin{solution}
    \begin{enumerate}
      ...
      \end{enumerate}
      \item CTR und RSA sind zusätzlich zu den obigen Eigenschaften auch homomorph.
      \item ElGamal kann kommutativ gemacht werden, indem der öffentliche Schlüssel $PK$ durch den geheimen Schlüssel $SK$ ersetzt wird. Eine mögliche Kommutativitätsdefinition könnte lauten: $EK_1(EK_2(M)) = EK_2(EK_1(M))$.
    \end{enumerate}
  \end{solution}
\end{exercise}
...
\begin{exercise}{Geburtstagsangriff auf CBC}
  ...
  \begin{solution}
    \begin{enumerate}
      \item Der Angreifer kann den Klartextblock $M2,1$ entschlüsseln, da $C2,1 = C3,1$ gilt. Der Angreifer berechnet $M2,1 = D_{SK}(C2,1 \oplus IV2) = D_{SK}(C3,1 \oplus IV3)$.
      \item Die Wahrscheinlichkeit, dass bei $m$ Ziehungen aus einer Menge von $n$ Zahlen mindestens zweimal die gleiche Zahl gezogen wird, beträgt $1 - \frac{n!}{(n-m)!n^m}$.
      \item Die Wahrscheinlichkeit, dass mindestens einmal Gleichheit von unterschiedlichen Schlüsseltextblöcken auftritt, beträgt $1 - \left(1 - \frac{1}{2^B}\right)^{N \cdot L}$.
      \item Die untere Grenze für die Anzahl an abzuhörenden Schlüsseltexten beträgt $\lceil \frac{\log(1 - q)}{L \cdot \log(1 - 1/2^B)} \rceil$.
      \item Die untere Grenze für die Anzahl an abzuhörenden Schlüsseltexten beträgt $\lceil \frac{\log(1 - q)}{2 \cdot L \cdot \log(1 - 1/2^B)} \rceil$.
      \item Die untere Grenze beträgt $2^{10}$.
      \item Die untere Grenze beträgt $2^{16}$.
      \item Der Angriff benötigt $2^{16} \cdot 64$ Bit Speicherplatz und $2^{16} \cdot 64$ Bit Zeit. Der Angriff ist aufgrund des hohen Speicherplatzbedarfs nicht realistisch durchführbar.
    \end{enumerate}
  \end{solution}
\end{exercise}
