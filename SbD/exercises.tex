...
\begin{exercise}{Meet-in-the-Middle-Angriff auf 2-DES}\label{ex:2-des}
...
\sheet[2024]{Mock exam}

\begin{exercise}{Kategorialer Vergleich von Verschlüsselungen}
  In dieser Aufgabe vergleichen Sie das One-Time-Pad, symmetrische Verschlüsselung unter Anwendung von Blockchiffren, asymmetrische Verschlüsselung und hybride Verschlüsselung (keine Verwendung des One-Time-Pads) hinsichtlich Sicherheit und Effizienz:.
  \begin{enumerate}
    \item Ordnen Sie die Verfahren hinsichtlich ihrer Sicherheit. Worauf basiert jeweils ihre Sicherheit?
    \item Ordnen Sie die Verfahren hinsichtlich ihrer Recheneffizienz. Welche Operationen werden jeweils verwendet und wie recheneffizient sind diese?
    \item Ordnen Sie die Verfahren hinsichtlich der Anzahl zu speichernder Schlüssel. Wie viele Schlüssel müssen gespeichert werden, damit anschließend in einer Gruppe jeder mit jedem mehrfach kommunizieren kann?
    \item Warum sind Ciphertext Only Attacks und Chosen Plaintext Attacks bei asymmetrischer Krypto praktisch dasselbe?
  \end{enumerate}
  ...
  \begin{solution}
    \begin{enumerate}
        \item Sicherheit:
          \begin{itemize}
              \item \textbf{One-Time-Pad:} Perfekte Sicherheit durch zufälligen Schlüssel.
              \item \textbf{Symmetrische Verschlüsselung:} Sicherheit basiert auf Geheimhaltung des Schlüssels.
              \item \textbf{Asymmetrische Verschlüsselung:} Sicherheit basiert auf Schwierigkeit des Faktorisierungsproblems.
              \item \textbf{Hybride Verschlüsselung:} Sicherheit basiert auf asymmetrischer Verschlüsselung und symmetrischer Schlüsselverteilung.
          \end{itemize}
        \item Recheneffizienz:
          \begin{itemize}
              \item \textbf{One-Time-Pad:} Sehr ineffizient, da Schlüssel genauso lang wie Nachricht.
              \item \textbf{Symmetrische Verschlüsselung:} Effizient, da Blockchiffren schnelle Verschlüsselung ermöglichen.
              \item \textbf{Asymmetrische Verschlüsselung:} Sehr ineffizient, da Berechnungen mit großen Zahlen.
              \item \textbf{Hybride Verschlüsselung:} Effizient, da asymmetrische Verschlüsselung nur für Schlüsselaustausch.
          \end{itemize}
        \item Anzahl Schlüssel:
          \begin{itemize}
              \item \textbf{One-Time-Pad:} Jeder benötigt eigenen zufälligen Schlüssel.
              \item \textbf{Symmetrische Verschlüsselung:} Ein gemeinsamer Schlüssel für alle.
              \item \textbf{Asymmetrische Verschlüsselung:} Jeder benötigt eigenen privaten Schlüssel.
              \item \textbf{Hybride Verschlüsselung:} Ein gemeinsamer Schlüssel für symmetrische Verschlüsselung.
          \end{itemize}
        \item Ciphertext Only Attacks und Chosen Plaintext Attacks sind bei asymmetrischer Krypto praktisch dasselbe, da der Angreifer den öffentlichen Schlüssel kennt und somit Plaintexte verschlüsseln kann.
    \end{enumerate}
  \end{solution}
\end{exercise}
...
\sheet[2020]{1st Exam}
\begin{exercise}{2-DES \ref{ex:2-des}}\end{exercise}

% One-Time-Pad
% a) Wie funktioniert ein OTP?
% b) Welche zwei Eigenschaften müssen erfüllt sein damit ein OTP Informationstheoretisch sicher ist.
% c) Können Quantencomputer OTPs besser knacken? Was unterscheidet ein OTP von anderen symetrischen oder asymetrischen Verfahren (z. B. RSA)?
% Begründung
% d) Person verschlüsselt ihre Backups mit den n ersten Zeichen eines auf einer DIN-A4 Seite ausgedruckten zufälligen Schlüssels und dem OTP. Warum ist dies unsicher?
% e) Gegeben Klartext m = "Sehr geherte Damen....", verschlüsslter Klartext c(m) in Binär und verschlüsselte PIN in Binär. PIN soll ermittelt werden. (ASCII-Tabelle gegeben)
\begin{exercise}{One-Time-Pad}
  \begin{enumerate}
    \item Wie funktioniert ein One-Time-Pad (OTP)?
    \item Welche zwei Eigenschaften müssen erfüllt sein, damit ein OTP informationstheoretisch sicher ist?
    \item Können Quantencomputer OTPs besser knacken? Was unterscheidet ein OTP von anderen symmetrischen oder asymmetrischen Verfahren (z. B. RSA)? Begründen Sie Ihre Antwort.
    \item Eine Person verschlüsselt ihre Backups mit den ersten $n$ Zeichen eines auf einer DIN-A4 Seite ausgedruckten zufälligen Schlüssels und dem OTP. Warum ist dies unsicher?
    \item Geben seien \begin{itemize}
        \item Klartext $m = \text{"HALLO"}$ % 7 0 11 11 14 
        \item Schlüsseltext $c(m) = 7\ 1\ 13\ 14\ 18$
        \item ASCII: $A=0, B=1, \ldots, Z=25$
      \end{itemize}
      Berechnen Sie die PIN.
  \end{enumerate}

  \begin{solution}
    \begin{enumerate}
        \item Ein OTP ist ein symmetrisches Verschlüsselungsverfahren, bei dem der Klartext mit einem zufälligen Schlüssel (dem One-Time-Pad) XOR-verknüpft wird.
        \item Ein OTP ist informationstheoretisch sicher, wenn der Schlüssel genauso lang wie der Klartext ist und der Schlüssel zufällig und geheim ist.
        \item Quantencomputer können OTPs nicht besser knacken, da der Schlüssel zufällig und geheim ist. Ein OTP unterscheidet sich von anderen Verfahren, da es perfekte Sicherheit bietet und nicht anfällig für Kryptoanalyse ist.
        \item Der Schlüssel ist nicht zufällig und geheim, da er auf einer DIN-A4 Seite ausgedruckt ist. Der Schlüssel ist nicht so lang wie der Klartext, was die Sicherheit des OTPs beeinträchtigt.
        \item Die PIN kann durch XOR-Verknüpfung des Klartexts mit dem Schlüsseltext berechnet werden: $7 \oplus 7 = 0, 1 \oplus 0 = 1, 13 \oplus 11 = 2, 14 \oplus 11 = 3, 18 \oplus 14 = 4$. Die PIN lautet also ABCDE.
    \end{enumerate}
  \end{solution}
\end{exercise}

\begin{exercise}{Public Key Infrastructure}
  \begin{enumerate}
    \item Erläutern Sie den Unterschied zwischen einer Signatur und einem Zertifikat.
    \item Beschreiben Sie das Zertifikatsmodell von TLS.
    \item Beschreiben Sie das Gültigkeitsmodell von TLS.
    \item Wie kann man große Dateien signieren? Welche Eigenschaften sollte dieses Konstrukt besitzen?
  \end{enumerate}

  \begin{solution}
    \begin{enumerate}
        \item Eine Signatur dient der Authentifizierung und Integritätsprüfung einer Nachricht. Ein Zertifikat enthält den öffentlichen Schlüssel eines Teilnehmers sowie die Signatur einer vertrauenswürdigen Zertifizierungsstelle (CA), die die Authentizität des Schlüssels bestätigt.
        \item Das Zertifikatsmodell von TLS basiert auf einer Zertifizierungskette, in der das Endzertifikat von einer vertrauenswürdigen Zertifizierungsstelle signiert wird. Der Browser vertraut der Wurzelzertifizierungsstelle und kann die Zertifikate der Website verifizieren.
        \item Das Gültigkeitsmodell von TLS basiert auf der Überprüfung der Zertifikate der Website durch den Browser. Der Browser vertraut der Wurzelzertifizierungsstelle und kann die Zertifikate der Website verifizieren.
        \item Große Dateien können signiert werden, indem ein Hash der Datei berechnet und dieser Hash mit dem privaten Schlüssel des Signierenden verschlüsselt wird. Die Signatur sollte unabhängig von der Dateigröße sein und die Integrität und Authentizität der Datei gewährleisten.
    \end{enumerate}
  \end{solution}
\end{exercise}

\begin{exercise}{Bluetooth}
  \begin{enumerate}
    \item Warum ist ein 4-stelliger PIN bei Bluetooth unsicher?
    \item Ein Angreifer versucht, die PIN, die beim Bluetooth-Handshake verwendet wird, zu erraten. In welchem Schritt kann ein Angreifer die PIN verifizieren?
    \item Wie muss ein passiver Angreifer vorgehen, um die PIN zu erraten?
  \end{enumerate}

  \begin{solution}
    \begin{enumerate}
        \item Ein 4-stelliger PIN ist unsicher, da er leicht durch Brute-Force-Angriffe geknackt werden kann.
        \item Ein Angreifer kann die PIN während des Pairing-Prozesses verifizieren, wenn die PIN eingegeben wird.
        \item Ein passiver Angreifer kann die PIN durch Abhören des Pairing-Prozesses und der Kommunikation zwischen den Geräten erraten.
    \end{enumerate}
  \end{solution}
\end{exercise}
